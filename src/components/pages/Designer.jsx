...
export default function Designer() {
  ...
  // --- React Query Hooks ---
  const { data: designerData, isLoading: tableLoading } = useDesigners(queryParams);
  const { data: metaData } = useDesignerMeta();
  const createMutation = useCreateDesigner();
  const updateMutation = useUpdateDesigner();
  const deleteMutation = useDeleteDesigner();

  const designers = designerData?.data?.data || [];
  const pagination = {
    current_page: designerData?.data?.current_page || 1,
    last_page: designerData?.data?.last_page || 1,
    total: designerData?.data?.total || 0,
    per_page: designerData?.data?.per_page || 10,
    from: designerData?.data?.from || 0,
    to: designerData?.data?.to || 0
  };
