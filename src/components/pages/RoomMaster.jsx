...
export default function RoomMaster() {
    ...
    // --- React Query Hooks ---
    const { data: roomResponse, isLoading: tableLoading } = useRooms(queryParams);
    const createMutation = useCreateRoom();
    const updateMutation = useUpdateRoom();
    const deleteMutation = useDeleteRoom();

    // Handle both direct array and paginated response
    const rooms = Array.isArray(roomResponse?.data) ? roomResponse.data : (roomResponse?.data?.data || []);
    const pagination = roomResponse?.data?.current_page ? {
        current_page: roomResponse?.data?.current_page || 1,
        last_page: roomResponse?.data?.last_page || 1,
        total: roomResponse?.data?.total || 0,
        per_page: roomResponse?.data?.per_page || 10,
        from: roomResponse?.data?.from || 0,
        to: roomResponse?.data?.to || 0
    } : {
        current_page: 1,
        last_page: 1,
        total: rooms.length,
        per_page: rooms.length || 10,
        from: 1,
        to: rooms.length
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50 p-6 font-sans text-slate-800">
            ...
            <motion.div
                ...
                className="mb-6"
            >
                <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Room Master</h1>
                <p className="text-sm text-slate-500 mt-1">Manage rooms for your listings</p>
            </motion.div>
                    ...
                    <div className="bg-white rounded-lg border border-slate-200 shadow-sm sticky top-6">
                        ...
                        <form onSubmit={handleSubmit} className="p-5 space-y-5">
                            ...
                            <div className="space-y-1.5">
                                <label className="block text-sm font-medium text-slate-700">Room Name <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                                    required
                                    value={roomName}
                            ...
                            <div className="space-y-1.5">
                                ...
                                >
                                    ...
                                    />
                                    {imagePreview ? (
                                        <div className="relative mx-auto w-32 h-32 rounded overflow-hidden shadow-sm border border-slate-200">
                            ...
                            <div className="pt-2">
                                ...
                                >
                                    ...
                                    {isFormLoading ? "Processing..." : (editingId ? "Update Room" : "Create Room")}
                                </button>
                            </div>
                        </form>
                    </div>
                        ...
                        >
                            ...
                            <div className="px-6 py-6">
                                <p className="text-slate-600 leading-relaxed">
                                    Are you sure you want to permanently delete this room? <br />
