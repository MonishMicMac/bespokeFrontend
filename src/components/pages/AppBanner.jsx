...
export default function AppBanner() {
  ...
  // --- React Query Hooks ---
  const { data: bannerData, isLoading: tableLoading } = useAppBanners(queryParams);
  const { data: metaData } = useAppBannerMeta();
  ...
  const pagination = {
    current_page: bannerData?.data?.current_page || 1,
    last_page: bannerData?.data?.last_page || 1,
    total: bannerData?.data?.total || 0,
    per_page: bannerData?.data?.per_page || 10,
    from: bannerData?.data?.from || 0,
    to: bannerData?.data?.to || 0
  };
        ...
        >
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm sticky top-6">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <button
                type="submit"
                disabled={isFormLoading || (!editingId && banners.length >= 5)}
